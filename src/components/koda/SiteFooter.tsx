...
export function SiteFooter({ dark = false }: { dark?: boolean }) {
  ...
  return (
    <footer className={`${dark ? "bg-black text-white" : "bg-[#f5f5f7] text-[#1d1d1f]"} border-t ${border}`}>
      <div className="mx-auto max-w-[1024px] px-5 pb-7 pt-7 sm:pt-9">
        <div className={`border-b ${border} pb-5 text-[10px] leading-[1.55] ${muted}`}>
          <p>
            Os recursos, disponibilidade e especificações podem variar conforme o modelo e a versão do KODA OS. Compras, garantia, KodaCare e assistência ficam vinculados à sua Conta Koda quando aplicável.
          </p>
        </div>
              ...
              <ul className="mt-3 space-y-2">
                {group.links.map(([label, href]) => (
                  <li key={`${group.title}-${href}`}>
                    <a href={href} className={`text-[10px] leading-none ${muted} transition-colors hover:text-current hover:underline`}>
                      {label}
                    </a>
                  </li>
                ))}
        ...
        <div className={`border-b ${border} pb-4 text-[10px] ${muted}`}>
          <p>
            Mais formas de comprar: visite a <a href="/loja" className="text-[#0066cc] hover:underline">Loja Koda</a> ou fale com a <a href="/suporte/contato" className="text-[#0066cc] hover:underline">equipe Koda</a>.
          </p>
        </div>

        <div className={`flex flex-col gap-3 pt-4 text-[10px] sm:flex-row sm:items-center sm:justify-between ${muted}`}>
          <p>Copyright © {new Date().getFullYear()} Koda Eletrônicos. Todos os direitos reservados.</p>
