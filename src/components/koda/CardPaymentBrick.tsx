...
export function CardPaymentBrick({ amountCents, enabled, orderRequest, existingOrderId, onOrderCreated }: Props) {
            ...
            onSubmit: (formData: any, additionalData: any) => new Promise<void>(async (resolve, reject) => {
              try {
                ...
                if (!orderId) {
                  ...
                  const { data: orderData, error: orderError } = await supabase.functions.invoke<{ order: { id: string; display_number?: string; status?: string } }>("koda-pay-create-order", { body: orderRequest });
  ...
  if (config && !config.card_ready) {
    return <div className="mt-6 rounded-2xl border border-black/10 bg-[#f5f5f7] p-5"><div className="flex gap-3"><CreditCard className="mt-0.5 h-5 w-5 shrink-0 text-[#0071e3]" /><div><p className="text-sm font-semibold">Cartão ainda não está disponível.</p><p className="mt-1 text-xs leading-relaxed text-[#6e6e73]">Use Pix por enquanto. Quando a Public Key do Mercado Pago estiver ativa, esta opção será liberada automaticamente.</p></div></div></div>;
  }
