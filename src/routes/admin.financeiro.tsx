...
function FinanceConsole() {
  ...
  async function load() {
    ...
    setLoading(true);
    setError(null);
    const { data: response, error: invokeError } = await supabase.functions.invoke<FinanceSummary>(
      "koda-pay-admin-summary",
      {
        body: {},
      },
    );
        ...
        <main className="grid min-h-[650px] place-items-center px-5 text-center">
          <div>
            ...
            <p className="mt-3 text-sm text-[#6e6e73]">
              Somente administradores da Koda podem acessar dados financeiros.
            </p>
          ...
          <div className="flex flex-col gap-6 sm:flex-row sm:items-end sm:justify-between">
            <div>
              ...
              <p className="mt-3 max-w-2xl text-sm leading-relaxed text-[#6e6e73]">
                Pedidos, pagamentos, estornos e catálogo financeiro em uma visão protegida pelo
                KodaCloud.
              </p>
            </div>
            <button
              onClick={load}
              disabled={loading}
              className="inline-flex h-10 items-center justify-center gap-2 rounded-full border border-black/10 px-4 text-sm font-semibold disabled:opacity-50"
            >
        ...
        {loading && !data ? (
          <div className="mt-4 grid min-h-[320px] place-items-center rounded-[32px] bg-white">
            <div className="text-center">
              ...
              <p className="mt-3 text-sm text-[#6e6e73]">Carregando dados financeiros…</p>
            </div>
          </div>
        ) : error ? (
          <div className="mt-4 rounded-[32px] bg-white p-8 text-center text-sm text-red-600">
            {error}
          </div>
        ) : data ? (
          <>
            <section className="mt-4 grid gap-4 sm:grid-cols-2 xl:grid-cols-4">
              <Metric
                ...
                note={`${data.metrics.awaiting_payment} aguardando pagamento`}
              />
              <Metric
                icon={Banknote}
                label="Receita confirmada"
                value={money(data.metrics.confirmed_revenue_cents)}
                note={`${data.metrics.paid_orders} pedidos pagos`}
              />
              <Metric
                icon={CreditCard}
                label="Pagamentos"
                value={String(data.payments.length)}
                note="Últimos 50 registros"
              />
              <Metric
                icon={RefreshCw}
                label="Estornado"
                value={money(data.metrics.refunded_cents)}
                note={`${data.refunds.length} solicitações recentes`}
              />
            </section>
                  ...
                  <article key={product.id} className="rounded-2xl border border-black/10 p-5">
                    <div className="flex items-start justify-between gap-4">
                      <div>
                        ...
                        <p className="mt-1 font-mono text-xs text-[#86868b]">{product.slug}</p>
                      </div>
                      <span
                        className={`rounded-full px-2.5 py-1 text-[11px] font-semibold ${product.active ? "bg-green-50 text-green-700" : "bg-[#f5f5f7] text-[#6e6e73]"}`}
                      >
                        {product.active ? "À venda" : "Desativado"}
                      </span>
                    </div>
            ...
            <section className="mt-4 rounded-[32px] bg-white p-6 sm:p-8">
              <div>
                <p className="text-sm font-semibold text-[#6e6e73]">Pedidos</p>
                <h2 className="mt-1 text-3xl font-semibold tracking-[-0.04em]">
                  Atividade recente.
                </h2>
              </div>
                ...
                <table className="w-full min-w-[860px] text-left text-sm">
                  <thead>
                    <tr className="border-b border-black/10 text-[11px] uppercase tracking-[0.12em] text-[#86868b]">
                      <th className="py-3 pr-5">Pedido</th>
                      <th className="py-3 pr-5">Cliente</th>
                      <th className="py-3 pr-5">Valor</th>
                      <th className="py-3 pr-5">Criado</th>
                    ...
                    {data.orders.map((order) => (
                      <tr key={order.id} className="border-b border-black/10">
                        <td className="py-4 pr-5 font-mono text-xs font-semibold">
                          KD-{String(order.order_number).padStart(6, "0")}
                        </td>
                        ...
                        <td className="py-4 pr-5 text-[#6e6e73]">{date(order.created_at)}</td>
                        <td className="py-4">
                          <span className="rounded-full bg-[#f5f5f7] px-2.5 py-1 text-[11px] font-semibold text-[#6e6e73]">
...
function Metric({
  ...
  return (
    <article className="rounded-[28px] bg-white p-6">
      ...
      <p className="mt-5 text-xs font-semibold uppercase tracking-[0.1em] text-[#86868b]">
        {label}
      </p>
      <p className="mt-2 text-3xl font-semibold tracking-[-0.04em]">{value}</p>
      <p className="mt-2 text-xs text-[#6e6e73]">{note}</p>
    </article>
  );
}
