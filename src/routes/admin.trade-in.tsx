...
function TradeInAdmin() {
  ...
  const load = useCallback(async () => {
    ...
    const { data } = await supabase
      ...
      .select("id,serial_number,source_model,credit_cents,status,outbound_tracking_code,created_at")
  ...
  async function updateStatus(id: string, status: string) {
    ...
    if (["approved", "rejected"].includes(status)) updates.inspected_at = new Date().toISOString();
                ...
                >
                  <div>
                    ...
                    <p className="mt-2 text-xs text-[#6e6e73]">
                      Crédito: R$ {(item.credit_cents / 100).toFixed(2).replace(".", ",")} ·{" "}
