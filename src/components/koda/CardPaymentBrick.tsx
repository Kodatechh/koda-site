    ...
    return (
      <div className="mt-6 rounded-2xl border border-black/10 bg-[#f5f5f7] p-5">
        <div className="flex gap-3">
          ...
          <div>
            <p className="text-sm font-semibold">Cartão preparado.</p>
            <p className="mt-1 text-xs leading-relaxed text-[#6e6e73]">Falta somente conectar a Public Key de teste da aplicação Mercado Pago.</p>
          </div>
        </div>
      </div>
    );
    ...
    <div className="mt-6 rounded-[24px] border border-black/10 bg-white p-5">
      <div className="mb-4 flex items-start gap-3">
        ...
        <div>
          ...
          <p className="mt-1 text-xs leading-relaxed text-[#6e6e73]">Os dados do cartão são tokenizados pelo Mercado Pago no navegador e não passam em formato bruto pelos servidores da Koda.</p>
        </div>
      </div>

      {!ready && !error && (
        <div className="mb-3 flex items-center gap-2 text-xs text-[#6e6e73]"><LoaderCircle className="h-4 w-4 animate-spin" />Carregando pagamento seguro…</div>
      )}
      ...
      {result?.status === "processed" && (
        <div className="mt-4 flex gap-3 rounded-2xl border border-green-200 bg-green-50 p-4 text-sm text-green-950">
          <CheckCircle2 className="h-5 w-5 shrink-0 text-green-700" />Pagamento aprovado. A confirmação também será registrada pelo webhook do Koda Pay.
        </div>
      )}
      ...
      {result?.challenge_url && (
        <div className="mt-5 overflow-hidden rounded-2xl border border-black/10">
          ...
          {challengeComplete && <p className="p-4 text-xs text-[#6e6e73]">Autenticação concluída. Aguardando a confirmação final do banco e do webhook.</p>}
        </div>
      )}

      {result && result.status !== "processed" && !result.challenge_url && (
        <p className="mt-4 text-xs font-medium text-[#6e6e73]">Status do pagamento: {result.status_detail ?? result.status}.</p>
      )}
