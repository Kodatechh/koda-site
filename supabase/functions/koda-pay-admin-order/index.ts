...
Deno.serve(async (req: Request) => {
  ...
  const trackingCode =
    typeof input.trackingCode === "string" ? input.trackingCode.trim().slice(0, 120) : "";

  if (!orderId || !["start_processing", "mark_shipped", "mark_delivered"].includes(action)) {
  ...
  if (action === "start_processing") {
    if (order.status !== "paid") {
      return json({ error: "invalid_order_transition", current_status: order.status }, 409);
    }
  ...
  await admin.from("admin_audit_log").insert({
    ...
    details: {
      ...
      tracking_code_set: Boolean(trackingCode),
    },
  });
