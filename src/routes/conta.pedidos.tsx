...
export const Route = createFileRoute("/conta/pedidos")({
  head: () => ({
    meta: [{ title: "Meus pedidos — Koda" }, { name: "robots", content: "noindex,nofollow" }],
  }),
  component: Orders,
});
...
function Orders() {
  ...
  useEffect(() => {
    ...
    db.from("orders")
      .select(
        "id,order_number,status,currency,total_cents,created_at,tracking_code,shipping_service,sales_mode,estimated_ship_start_at,order_items(id,product_name,quantity,total_amount_cents)",
      )
      .eq("user_id", user.id)
    ...
    return (
      <Main>
        <div className="rounded-[32px] bg-white p-10 text-center">
          <h1 className="text-3xl font-semibold tracking-[-.04em]">Entre para ver seus pedidos.</h1>
          <a
            href="/conta/entrar?returnTo=/conta/pedidos"
  ...
  return (
    <Main>
      ...
      <p className="mt-4 max-w-xl text-sm leading-relaxed text-[#6e6e73]">
        Pagamento, preparação, envio, rastreio e documentação ficam reunidos aqui.
      </p>
      ...
      <div className="mt-10 space-y-4">
        {items.map((o) => (
          <a
            ...
            className="group block rounded-[28px] bg-white p-6 transition hover:-translate-y-0.5 hover:shadow-[0_14px_40px_rgba(0,0,0,.06)] sm:p-7"
          >
            <div className="flex flex-wrap items-start justify-between gap-5">
              <div className="min-w-0">
                ...
                {o.sales_mode === "preorder" && (
                  <p className="mt-2 inline-flex rounded-full bg-[#eef6ff] px-3 py-1 text-[10px] font-semibold text-[#0066cc]">
                    Pré-venda · envios a partir de 17/10
                  </p>
                )}
                ...
                <div className="mt-4 flex flex-wrap gap-x-3 gap-y-1 text-sm text-[#6e6e73]">
                  {o.order_items.map((i) => (
                    <span key={i.id}>
                ...
                {o.tracking_code && (
                  <div className="mt-4 inline-flex items-center gap-2 rounded-full bg-[#f5f5f7] px-3 py-1.5 text-xs font-semibold">
              ...
              <div className="flex items-center gap-4">
                <strong className="text-lg tracking-[-.02em]">
                  {new Intl.NumberFormat("pt-BR", {
                    style: "currency",
                    currency: o.currency,
                  }).format(o.total_cents / 100)}
                </strong>
                <ChevronRight className="h-5 w-5 text-[#86868b] transition group-hover:translate-x-1 group-hover:text-[#0071e3]" />
              </div>
            </div>
          </a>
        ))}
        ...
        {!items.length && (
          <div className="rounded-[28px] bg-white p-12 text-center">
            ...
            <p className="mx-auto mt-2 max-w-sm text-sm text-[#6e6e73]">
              Quando você comprar um produto Koda, ele aparecerá aqui para acompanhamento.
            </p>
            <a
              href="/loja"
              className="mt-6 inline-flex rounded-full bg-[#0071e3] px-5 py-2.5 text-sm font-semibold text-white"
            >
              Ir para a Loja Koda
            </a>
          </div>
